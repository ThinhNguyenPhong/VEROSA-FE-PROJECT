...
import { login } from '../apis/accountApi/loginApi';
...
const LoginForm = () => {
  ...
  return (
    <div className="min-h-screen w-screen flex items-center justify-center bg-gradient-to-br from-[#FFCC66]/30 via-white to-[#6C63FF]/10 overflow-hidden relative">
      ...
      >
        <h2 className="text-3xl font-bold text-center mb-6 text-[#FFCC66]">Đăng nhập</h2>
        {success && <div className="text-green-600 text-center mb-4">{success}</div>}
        {error && <div className="text-red-600 text-center mb-4">{error}</div>}
        <div className="grid grid-cols-1 gap-4">
          <input name="usernameOrEmail" value={form.usernameOrEmail} onChange={handleChange} placeholder="Tên đăng nhập hoặc Email" className="border rounded px-3 py-2 focus:ring-2 focus:ring-[#FFCC66]" required />
          <input name="password" value={form.password} onChange={handleChange} type="password" placeholder="Mật khẩu" className="border rounded px-3 py-2 focus:ring-2 focus:ring-[#FFCC66]" required />
        </div>
        <button
          type="submit"
